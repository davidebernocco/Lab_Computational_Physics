...
@njit
def RW2D_average(N_w, N, x0, y0, Pl, Pr, Pd):
    ...
    for j in range(N_w):
        ...
        lst_l = np.random.uniform(0, 1, N) #if I want make it continuous
...
def RW_2D_plot(Opera):
    ...
    plt.grid(True)
    #plt.gca().set_xticks([i for i in range(np.min(Opera[0]) - 1, np.max(Opera[0]) + 1,1)]) 
    #plt.gca().set_yticks([i for i in range(np.min(Opera[1]) - 1, np.max(Opera[1]) + 1,1)]) 
